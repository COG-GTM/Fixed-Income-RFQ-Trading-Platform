package com.javieraviles.counterpartyservice.controller;
...
import com.javieraviles.counterpartyservice.dto.OperationEnum;
...
import com.javieraviles.counterpartyservice.exception.ResourceNotFoundException;
import com.javieraviles.counterpartyservice.repository.CounterpartyRepository;
import com.javieraviles.counterpartyservice.restclient.TradeConfirmationMicroserviceClient;
import com.javieraviles.counterpartyservice.service.TradeConfirmationService;
...
class CounterpartyController {
        ...
        ResponseEntity<Counterparty> getOne(@PathVariable Long id) {
                final Counterparty counterparty = repository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException());
                return ResponseEntity.ok(counterparty);
        }

        @PutMapping("/counterparties/{id}")
        ResponseEntity<Counterparty> updateCounterparty(@RequestBody Counterparty updatedCounterparty,
                ...
                try {
                        ...
                        if (operation == OperationEnum.ADD) {
                                ...
                                final TradeConfirmationDto confirmation = new TradeConfirmationDto(
                                                counterparty.getName(), creditAmount);
        ...
        @PostMapping("/counterparties/{id}/validate")
        public ResponseEntity<Counterparty> validateCounterparty(@PathVariable Long id) {
                final Counterparty counterparty = repository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException());
                return ResponseEntity.ok(counterparty);
        }

        @PostMapping("/counterparties/{id}/deduct-credit")
        public ResponseEntity<Counterparty> deductCredit(@PathVariable Long id,
                        @RequestBody CreditDeductionRequest request) {
                final Counterparty counterparty = repository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException());
                counterparty.deductCredit(request.getAmount());
                return ResponseEntity.ok(repository.save(counterparty));
        }
