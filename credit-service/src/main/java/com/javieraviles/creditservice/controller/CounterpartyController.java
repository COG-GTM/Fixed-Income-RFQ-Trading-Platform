...
/**
 * Counterparty management owned by the extracted credit service. Mirrors the
 * monolith's counterparty CRUD so the service can be seeded and administered
 * independently.
 */
@RestController
class CounterpartyController {
	...
	CounterpartyController(final CounterpartyRepository repository) {
	...
	ResponseEntity<Counterparty> getOne(@PathVariable Long id) {
		final Counterparty counterparty = repository.findById(id).orElseThrow(ResourceNotFoundException::new);
